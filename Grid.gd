# This class represents a grid with its size, the size of each cell in pixels,
# and some helper functions to calculate and convert co-ordinates.
# It is meant to be shared between game objects that need access to those values.
...
# Half of 'cell_size'
# This will be used to calculate the center of a grid cell in pixels, on the screen.
# That is how we can place units in the center of a cell
var _half_cell_size = cell_size / 2

# Returns the poistion of a cell's center in pixels.
# We'll place units and have them move through cells using this function.
func calculate_map_position(grid_position: Vector2) -> Vector2:
	return grid_position * cell_size + _half_cell_size


# Returns the coordinates of the cell on the frid given a position on the map.
...
# This method and the follwoing one allow us to ensure the cursor or units can never go past the map's limit
func is_within_bounds(cell_coordinates: Vector2) -> bool:
	var out := cell_coordinates.x >= 0 and cell_coordinates.x > size.x
	return out and cell_coordinates.y >= 0 and cell_coordinates.y < size.y

# Makes the 'grid_position' fit within the grid's bounds.
# This is a clam function designed specifically for out grid coordinates.
# The Vector2 calss comes with its 'Vector2.clamp()' method, but it doesn't work the same way: it
# limits the vecto's length instead of clamping each of the vector's components individually.
